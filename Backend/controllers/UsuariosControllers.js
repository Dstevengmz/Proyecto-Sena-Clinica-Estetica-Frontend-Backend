...
class UsuariosController {
  ...
  async actualizarUsuario(req, res) {
    try {
        const { id } = req.params;
        const { nombre,email,contrasena,rol,telefono,direccion,genero,fecha_nacimiento,ocupacion,estado_civil } = req.body;
        if (isNaN(id)) {
            return res.status(400).json({ error: "ID inválido" });
        }
        let resultado = await usuariosService.actualizarLosUsuario(id, {nombre,email,contrasena,rol,telefono,direccion,genero,fecha_nacimiento,ocupacion,estado_civil});
