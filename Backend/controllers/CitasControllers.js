...
class CitasControllers {
  ...
  async cancelarCita(req, res) {
    try {
      ...
      await citasService.actualizarLasCitas(id, { estado: "cancelada" });

      if (cita.id_doctor) {
        await citasService.notificarTotalCitasCanceladas(cita.id_doctor);
        await citasService.notificarTotalCitasPendientesHoy(cita.id_doctor);
        await citasService.notificarTotalCitasCanceladasHoy(cita.id_doctor);
      ...
      res
        .status(500)
        .json({ error: "Error en el servidor al cancelar la cita" });
    }
  }
  ...
  async crearCitas(req, res) {
    try {
      ...
      if (nuevocitas && nuevocitas.id_doctor) {
        ...
        await citasService.notificarTotalCitasPendientesHoy(
          nuevocitas.id_doctor
        );
        await citasService.notificarTotalCitasCanceladasHoy(
          nuevocitas.id_doctor
        );

        if (nuevocitas.tipo === "evaluacion") {
          await citasService.notificarTotalCitasRealizadasEvaluacionHoy(
            nuevocitas.id_doctor
          );
        } else if (nuevocitas.tipo === "procedimiento") {
          await citasService.notificarTotalCitasRealizadasProcedimientoHoy(
            nuevocitas.id_doctor
          );
        }
  ...
  async actualizarCitaDoctor(req, res) {
    try {
      ...
      if (cita && cita.id_doctor) {
        ...
        } else if (cita.tipo === "procedimiento") {
          await citasService.notificarTotalCitasRealizadasProcedimientoHoy(
            cita.id_doctor
          );
        }
  ...
  async generarPDFMedicamentos(req, res) {
    try {
      const { id } = req.params;
      const cita = await citasService.buscarLasCitas(id);
      if (!cita) return res.status(404).json({ error: "Cita no encontrada" });
      if (!cita.medicamentos_recetados)
        return res
          .status(400)
          .json({ error: "La cita no tiene medicamentos recetados" });
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="receta_medicamentos_cita_${id}.pdf"`
      );
      const doc = new PDFDocument({ margin: 40 });
      doc.pipe(res);
      ...
      doc
        ...
        .fontSize(13)
        .fillColor("#000")
        .text("RECETA MÉDICA", startX, y, {
          width: ancho - 110,
          align: "center",
        });
      y = doc.y + 8;

      doc.font("Helvetica-Bold").fontSize(10).text("Se recetan:", 55, y);
      y = doc.y + 4;
      const medLines = cita.medicamentos_recetados
        .split(/\r?\n+/)
        .filter((l) => l.trim());
      doc.font("Helvetica").fontSize(11).fillColor("#111");
      medLines.forEach((linea, idx) => {
        const safe = linea.trim();
        doc.text(`${idx + 1}. ${safe}`, { width: ancho - 110, align: "left" });
      });
      y = doc.y + 12;
      ...
      console.error("Error generando PDF medicamentos:", e);
      res.status(500).json({ error: "Error al generar PDF" });
    }
  }
